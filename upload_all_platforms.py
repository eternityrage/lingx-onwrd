...
def get_latest_reel():
    ...
    return {"video_path": str(latest), "metadata": metadata, "words": words_data, "all_words": all_words, "word": all_words[0] if all_words else "Word"}

def generate_caption(reel_data, platform="facebook"):
    words = reel_data.get("words", [])
    if not words:
        return f"Powerful words with {CHANNEL_NAME}! #LingexaOneWord"
    if platform == "facebook":
        lines = [f"📖 One Word That Replaces a Whole Sentence! with {CHANNEL_NAME}", f""]
        for i, w in enumerate(words, 1):
            ...
            lines.append(f"")
        lines.extend([f"💡 Save this to grow your vocabulary!", f"🔔 Follow {CHANNEL_NAME}!", f"", f"#LingexaOneWord #Vocabulary #LearnEnglish #PowerWords #WordPower #ESL #SAT #GRE #EnglishVocabulary"])
    else:
        lines = [f"📖 One Word That Replaces a Whole Sentence!", f""]
        for i, w in enumerate(words[:3], 1):
            lines.append(f"{i}. {w.get('word', '')} - {w.get('definition', '')}")
        lines.extend([f"", f"#LingexaOneWord #Vocabulary #LearnEnglish"])
    return "\n".join(lines)
        ...
        if upload_func:
            try:
                if platform_name == "facebook":
                    upload_result = upload_func(video_path=video_path, description=caption, title=f"Power Word: {word}")
...
def main():
    ...
    if not reel:
        print("No reel found! Run lingexa_oneword_bot.py first.")
        sys.exit(1)
